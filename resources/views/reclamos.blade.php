    ...
    <div class="container">
      ...
      <br><br>
        <form action="{{route('reclamos.busqueda')}}" method="GET" class="d-flex" role="search">
            <label for="busqueda" hidden>Formulario de búsqueda:></label><input class="form-control me-2" id="busqueda" type="search" name="valores" placeholder="Número de medidor, planilla o apellidos" aria-label="Buscar" required>
        ...
        <table id="tabla" class="table caption-top table-hover table-responsive table table-bordered table-striped table-sm">
          ...
          <tbody>
            ...
            <tr>
             ...
             @if($reclamosItem->estado_reclamo == "ingresado")<td class="td_acciones"><span class="badge mt-1 text-bg-info">{{ucfirst($reclamosItem->estado_reclamo);}}</span></td>@endif
              @if($reclamosItem->estado_reclamo == "en proceso")<td class="td_acciones"><span class="badge mt-1 text-bg-primary">{{ucfirst($reclamosItem->estado_reclamo);}}</span></td>@endif
              @if($reclamosItem->estado_reclamo == "resuelto")<td class="td_acciones"><span class="badge mt-1 text-bg-success">{{ucfirst($reclamosItem->estado_reclamo);}}</span></td>@endif
             ...
             @if($reclamosItem->estado_reclamo == "en proceso")
                  <!--BOTON ACTUALIZAR ESTADO A MARCAR COMO RESUELTO-->
                   ...
                   <!--BOTON ACTUALIZAR ESTADO A MARCAR COMO EN PROCESO-->
                      <a type="button" href="{{route('reclamos.actualizarEstado', $reclamosItem)}}" class="btn btn-outline-dark" title="Marcar como en proceso"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">
                   ...
                   <!--BOTON ACTUALIZAR ESTADO A MARCAR COMO INGRESADO-->
                      <a type="button" href="{{route('reclamos.actualizarEstado', $reclamosItem)}}" class="btn btn-outline-info" title="Marcar como ingresado"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard-plus" viewBox="0 0 16 16">
                      ...
                      $('#tabla').DataTable({
                          ...
                          buttons: [
                              ...
                              },
                              {
                                  ...
                                  title: '<center>LISTADO DE RECLAMOS</center>',
