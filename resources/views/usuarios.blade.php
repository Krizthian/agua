           ...
           <!--BOTON EDITAR-->
            ...
            <!--BOTON ELIMINAR-->
                     <div class="btn-group"><form action="{{route('usuarios.destroy', $usuariosItem )}}" method="POST">
                      @csrf @method('DELETE')
                      <button title="Eliminar usuario" class="btn btn-outline-danger" onclick="return confirm('¿Estás seguro de que deseas eliminar al usuario {{$usuariosItem->usuario}}?')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
